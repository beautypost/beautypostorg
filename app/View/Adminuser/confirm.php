...
<form class="form-horizontal" role="form" action="<?php echo WEBROOT.$this->name?>/input">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
        <span class="confirmtext"><?php echo $data['News']['title']?></span>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1">URL </label>
        <div class="col-sm-10">
            <span class="confirmtext"><?php echo $data['News']['url']?></span>
        </div>
    </div>
    </form>
...
<form method="post" action="<?php echo WEBROOT.$this->name?>/input" style="display:inline">
    ...
    <input type="hidden" name="data[News]" value="<?php echo base64_encode(serialize($data['News'])) ?>">
    </form>
